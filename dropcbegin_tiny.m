...
handles.dropcProg.output_file='C:\Users\Diego Restrepo\Desktop\FCM\test1.mat';
...
handles.dropcProg.splusOdorValve=uint8(1); %Make sure to use int1
handles.dropcProg.splusName='EA';


%Enter S- valve (1,2,4,8,16,32,64,128) and odor name
%FOR BEGIN THIS DOES NOT MAKE A DIFFERENCE
handles.dropcProg.sminusOdorValve=uint8(2); %Make sure to use int2
...
%dio ports for input
handles.dropcProg.lick_dioin=5;
handles.dropcProg.photodiode_dioin=6;
...
%Final valve and water valve
handles.dropcProg.finalValve=uint8(8);
handles.dropcProg.waterValve=uint8(4);
...
if run_program==1
    ...
    handles=dropcInitializePortsNowTiny(handles);
    ...
    if handles.begin.initStage==1
        ...
        handles=dropcStageOneTiny(handles);
        handles.elapsedStage1=toc;
    end
    
    handles.startStage2=toc;
    handles=dropcStageTwoTiny(handles);
