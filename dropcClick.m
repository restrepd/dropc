...
%    $Revision: 1.5.2.7 $  $Date: 2005/06/27 22:35:12 $
...
% data = 2*sin(linspace(0,2*pi*4000,len))';
% 
% %Two 5 msec clicks
% % onesec=0.1*ActualRate;
% % fivems=0.005*ActualRate;
% % pointfive=0.5*ActualRate;
% % data(1:onesec)=0;
% % data(onesec+1:onesec+fivems)=rand(1,fivems);
% % data(onesec+fivems+2:onesec+fivems+pointfive)=0;
% % data(onesec+fivems+pointfive+1:onesec+2*fivems+pointfive)=rand(1,fivems);
% % data(onesec+2*fivems+pointfive+1)=0;
% % data=2*data';
% 
% putdata(AO,data)
% 
...
%Notify draq of click
handles.dropcDigOut.draqPortStatus=uint8(8);
