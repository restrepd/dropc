...
handles.dropcProg.output_file='C:\Users\Restrepo\Desktop\DEMJDRG\Mouse Urine Program/m01.mat';
...
handles.dropcProg.splusName='2-heptanone';


%Enter S- valve (1,2,4,8,16,32,64,128) and odor name
handles.dropcProg.sminusOdorValve=uint8(4); %Make sure to use int8
handles.dropcProg.sminusName='2-heptanone + 3-heptanone';

%Enter first distractor trial
handles.dropcProg.firstDistractorTrial=1000;

%Enter the distractor after 
handles.dropcProg.distractorValve=uint8(8); %Make sure to use int8
handles.dropcProg.distractorName='Estrous urine';

%Enter the before distractor
handles.dropcProg.befDistractorValve=uint8(32); %Make sure to use int8
handles.dropcProg.befDistractorName='Water';
...
handles.dropcProg.timePerTrial=5;
...
if run_program==1
    ...
    while (stopTrials==0)&(handles.dropcData.trialIndex<200)
        ...
        if (handles.dropcProg.randomFellows(handles.dropcData.fellowsNo) == 1)
            ...
            %For only in S-
%             handles.dropcProg.secondValve=handles.dropcProg.befDistractorValve;
        else
            %S- odor
            ...
            %For only in S-
%             if handles.dropcData.trialIndex>handles.dropcProg.firstDistractorTrial
%                 handles.dropcProg.secondValve=handles.dropcProg.distractorValve;
%             else
%                 handles.dropcProg.secondValve=handles.dropcProg.befDistractorValve;
%             end
        end
        ...
        while (resultOfTrial == -2)
            ...
            if (dropcFinalValveOKurine(handles)==1)
