%dropcspm_hf_fsds
%The user has the choice to have the go-no go be self-initiated or not
%% Close all
clear all
close all
...
while (stopTrials==0)&(handles.dropcData.trialIndex<200)
    ...
    if handles.dropcProg.self_initiated==1
        ...
        this_rand=rand;
        while (toc-last_trial_start)<handles.dropcProg.minITI+this_rand*(handles.dropcProg.maxITI-handles.dropcProg.minITI)
        end
