...
%% This file runs an spm with different concentrations of odor following
% the protocol in Wojcik and Sirotin, Current Biology 24, 568?573, March 3, 2014
% There are three S+ and three S-
% Three S+ = 10%, 3.165%, 1%
% Three S- = 0.3165%, 0.1%, 0.03165%
% Ammonia is used to punish the animal for the incorect response on S-
...
%Valve for ammonia
handles.dropcProg.punishV=uint8(64); %Make sure to use uint8
...
handles.dropcDraqOut.reinforcement=uint8(32);

%Set the numbers for digital output to olfactometer DIO96H/50
%These are the values to control the valves
...
%Get the random numbers for which high/low odor concentration is chosen
handles.dropcProg.randomSplus=ceil(10*rand(1,200)/3.3333);
handles.dropcProg.nSplus=0;

handles.dropcProg.randomSminus=ceil(10*rand(1,200)/3.3333)+3;
handles.dropcProg.nSminus=0;
...
if run_program==1
    ...
    while (stopTrials==0)&(handles.dropcData.trialIndex<200)
        ...
        if (handles.dropcProg.randomFellows(handles.dropcData.fellowsNo) == 1)
            %S+ odor
            handles.dropcProg.nSplus=handles.dropcProg.nSplus+1;
            handles.dropcProg.odorValve=handles.dropcProg.odorV(handles.dropcProg.randomSplus(handles.dropcProg.nSplus));
            handles.dropcProg.odorValveNo=uint8(handles.dropcProg.randomSplus(handles.dropcProg.nSplus)+1);
            handles.dropcProg.typeOfOdor=handles.dropcProg.splusOdor;
            disp(['Trial No: ' num2str(handles.dropcData.trialIndex) '; S+, odor No =' num2str(handles.dropcProg.randomSplus(handles.dropcProg.nSplus))])
        else
            %S- odor
            handles.dropcProg.nSminus=handles.dropcProg.nSminus+1;
            handles.dropcProg.odorValve=handles.dropcProg.odorV(handles.dropcProg.randomSminus(handles.dropcProg.nSminus));
            handles.dropcProg.odorValveNo=uint8(handles.dropcProg.randomSminus(handles.dropcProg.nSminus)+1);
            handles.dropcProg.typeOfOdor=handles.dropcProg.sminusOdor;
            disp(['Trial No: ' num2str(handles.dropcData.trialIndex) '; S-, odor No =' num2str(handles.dropcProg.randomSminus(handles.dropcProg.nSminus))])
        end
        ...
        while (resultOfTrial == -2)
            ...
            if (dropcFinalValveOK_conc(handles)==1)
                %This mouse stayed on during the final valve; do the
                %single trial!


                trialResult=dropcDoesMouseRespondNow_conc(handles);
                ...
                if (trialResult~=2)
                    ...
                    dropcReinforceAppropriatelyPunish(handles);
                    ...
                    handles.dropcDigOut.draqPortStatus=handles.dropcProg.odorValveNo;
